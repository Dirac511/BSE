...
class BSE:
    def __init__(self,model,Tval,fileG4,fileG,file_analysis_hdf5,draw,useG0,symmetry_layer,symmetrizeG4,phSymmetry,calcRedVertex,calcCluster,build_sym_kernel,useGamma_hdf5,nkfine,compare_with_analysishdf5,write_data_file):
    ...
    def buildChi0Lattice(self,nkfine):
        ...
        self.chi0DXY  = np.zeros((nw,nk,nOrb,nOrb,nw,nk,nOrb,nOrb),dtype='complex')
        self.chi0DXY2  = np.zeros((nw,nk,nOrb,nOrb,nw,nk,nOrb,nOrb),dtype='complex')
        ...
        for iwn,wn in enumerate(wnSet): # reduced tp frequencies !!
           # print("iwn = ",iwn)
            ...
            for iK,K in enumerate(Kset):
                ...
                c6 = np.zeros((nOrb,nOrb,nOrb,nOrb),dtype='complex')
                c7 = np.zeros((nOrb,nOrb,nOrb,nOrb),dtype='complex')
                ...
                self.chi0DXY[iwn,iK,:,:,iwn,iK,:,:]  = c6[:,:,:,:]/kPatch.shape[0]
    ...
    def trans_basis(self,evecs):  ### transform the orbital-basis eigenvector into band basis
        ...
        if self.model == 'trilayer':
            ...
            tmat[0, 0] = 0.5*sqrt(2);  tmat[0, 1] = 0.5*sqrt(2)  # Bond
            tmat[1, 0] = 0.5*sqrt(2);  tmat[1, 1] = -0.5*sqrt(2)  # Anti
       
        elif self.model == 'La3Ni2O7':
            tmat=np.zeros((4,4),dtype=np.float64)
            tmat[0,0] = 0.5*sqrt(2); tmat[0,1] = 0;           tmat[0,2] = 0.5*sqrt(2);  tmat[0,3] = 0            #Bondx
            tmat[1,0] = 0;           tmat[1,1] = 0.5*sqrt(2); tmat[1,2] = 0;            tmat[1,3] = 0.5*sqrt(2)  #Bondz    
            tmat[2,0] = 0.5*sqrt(2); tmat[2,1] = 0;           tmat[2,2] = -0.5*sqrt(2); tmat[2,3] = 0            #Antix
            tmat[3,0] = 0;           tmat[3,1] = 0.5*sqrt(2); tmat[3,2] = 0;            tmat[3,3] = -0.5*sqrt(2) #Antiz
            ...
            for ilam in range(0,16):
                    for iNc in range(Nc):
                        ...
                        if self.model == 'trilayer':
                             ...
                             self.write_data_7cols(fname, kxs, kys, self.wnSet[int(NwG4/2)], targetVec[int(NwG4/2):int(NwG4/2)+1,iNc,0,0,ilam], targetVec[int(NwG4/2):int(NwG4/2)+1,iNc,1,1,ilam],targetVec[int(NwG4/2):int(NwG4/2)+1,iNc,2,2,ilam],targetVec[int(NwG4/2):int(NwG4/2)+1,iNc,3,3,ilam])
    ...
    def calcKernelEigenValues(self):
        ...
        if self.model == 'bilayer' or self.model == 'trilayer' or self.model == 'La3Ni2O7':
...
########################################################################
    def calcClusterSus(self):
        ...
        NwG = self.NwG;
        nOrb = self.nOrb
        chi_c0 = np.zeros((nOrb, nOrb), dtype='complex')
        chi_cpi = np.zeros((nOrb, nOrb), dtype='complex')
        self.G4k = np.zeros((nOrb, nOrb, nOrb, nOrb), dtype='complex')

        for l1 in range(nOrb):
            for l3 in range(nOrb):
                chi_c0[l1, l3] = sum(self.G4r[:, :, l1, l1, :, :, l3, l3])
                chi_c0[l1, l3] /= self.Nc * self.invT
                #print(l1,l3,': ',chi_c0[l1,l3])
        '''
        if self.model == 'La3Ni2O7':
            for l1 in range(0,nOrb):
                for l3 in range(0,nOrb):
                    if abs(l1-l3)==2: 
                        chi_cpi[l1,l3] = sum(self.G4r[:, :, l1, l1, :, :, l3, l3]) * exp(1j*np.pi*abs(l1-l3)/2.)/(self.Nc * self.invT)/2.0
                    elif (l1==1 and l3==2) or (l1==2 and l3==1) or (l1==0 and l3==3) or (l1==3 and l3==0):
                        chi_cpi[l1,l3] = sum(self.G4r[:, :, l1, l1, :, :, l3, l3]) * exp(1j*np.pi*abs(l1-l3))/(self.Nc * self.invT)/2.0
                    else:
                        chi_cpi[l1,l3] = sum(self.G4r[:, :, l1, l1, :, :, l3, l3])/(self.Nc * self.invT)/2.0
        '''
        for l1 in range(self.nOrb):
            for l2 in range(self.nOrb):
                for l3 in range(self.nOrb):
                    for l4 in range(self.nOrb):
                       self.G4k[l1,l2,l3,l4] = sum(self.G4r[:, :, l1, l2, :, :, l3, l4])

        a = np.array([1/sqrt(2),0,1/sqrt(2),0])  # Bonding   x
        b = np.array([0,1/sqrt(2),0,1/sqrt(2)])  # Bonding   z
        c = np.array([1/sqrt(2), 0, -1/sqrt(2), 0])  # antiBonding  x
        d = np.array([0, 1/sqrt(2), 0, -1/sqrt(2)])  # AntiBonding  z
        ...
        for i in range(0, len(basis)):
            for j in range(0, len(basis)):
                ...
                if i == 3:
                    M4.append(mij)
        Matrix = np.array([M1, M2, M3, M4], dtype=object)

        m1 = M1[0]+M1[1]+M2[0]+M2[1]
        m2 = M3[2]+M3[3]+M4[2]+M4[3]

        Mat = np.array([m1,m2],dtype=object)

        self.G4K = np.zeros((2, 2, 2), dtype='complex')
        for l1 in range(self.nOrb):
            for l2 in range(self.nOrb):
                for l3 in range(self.nOrb):
                    for l4 in range(self.nOrb):
                        for iqz in range(0,1):
                            if self.vertex_channel in ("PARTICLE_HOLE_MAGNETIC"):
                                self.G4K[0, 0, iqz] += (self.G4k[l1, l2, l3, l4] * Matrix[0][0][l1][l2] * Matrix[0][0][l3][l4] + \
                                                        ...
                                                        self.G4k[l1, l2, l3, l4] * Matrix[1][1][l1][l2] * Matrix[1][1][l3][l4])*Mat[iqz][l2][l4]
                                ...
                                self.G4K[0, 1, iqz] += (self.G4k[l1, l2, l3, l4] * Matrix[0][0][l1][l2] * Matrix[2][2][l3][l4] + \
                                                        ...
                                                        self.G4k[l1, l2, l3, l4] * Matrix[1][1][l1][l2] * Matrix[3][3][l3][l4])*Mat[iqz][l2][l4]
                                ...
                                self.G4K[1, 0, iqz] += (self.G4k[l1, l2, l3, l4] * Matrix[2][2][l1][l2] * Matrix[0][0][l3][l4] + \
                                                        ...
                                                        self.G4k[l1, l2, l3, l4] * Matrix[3][3][l1][l2] * Matrix[1][1][l3][l4])*Mat[iqz][l2][l4]
                                ...
                                self.G4K[1, 1, iqz] += (self.G4k[l1, l2, l3, l4] * Matrix[2][2][l1][l2] * Matrix[2][2][l3][l4] + \
                                                        ...
                                                        self.G4k[l1, l2, l3, l4] * Matrix[3][3][l1][l2] * Matrix[3][3][l3][l4])*Mat[iqz][l2][l4]
        ...
        print("cluster susceptibility, intra dx2y2 qz=0", ": T = ", self.Tval, ' ', real(chi_c0[0, 0]))
        print("cluster susceptibility, intra dz2 qz=0", ": T = ", self.Tval, ' ',real(chi_c0[1, 1]))
        print("cluster susceptibility, intra dxz qz=0", ": T = ", self.Tval, ' ',real(chi_c0[0, 1]))
        print("cluster susceptibility, inter dx2y2 qz=0", ": T = ", self.Tval, ' ',real(chi_c0[0, 2]))
        print("cluster susceptibility, inter dz2 qz=0", ": T = ", self.Tval, ' ',real(chi_c0[1, 3]))
        print("cluster susceptibility, inter dxz qz=0", ": T = ", self.Tval, ' ',real(chi_c0[1, 2]))
    ...
    def calcSCClusterSus(self):
        ...
        csumspmc = np.zeros((nOrb, nOrb, nOrb, nOrb), dtype='complex')
        ...
        G4d = (Pd[0, 0] + Pd[2, 2] + Pd[1, 1] + Pd[3, 3])/2. ## sum dx2y2, dz2
        G4d1 = (Pd[0, 0] + Pd[2, 2])/2. ## only dx2y2
        G4d2 = (Pd[1, 1] + Pd[3, 3])/2. ## only dz2 
        '''
        print("G4 d cluster Pairfield susceptibility Total: ", real(G4d))
        print("G4 d cluster Pairfield susceptibility dx2y2: ", real(G4d1))
        print("G4 d cluster Pairfield susceptibility dz2  : ", real(G4d2))
        '''
        ...
        '''     
        if self.model == 'La3Ni2O7':
            Pspmc1 = 0;Pspmc2 = 0;Pspmc = 0;Pdc1 = 0;Pdc2 = 0;Pdc = 0
            G4spm = 0.25 * (self.G4r[:, :, 0, 2, :, :, 0, 2] + self.G4r[:, :, 0, 2, :, :, 2, 0] \
                           + self.G4r[:, :, 2, 0, :, :, 0, 2] + self.G4r[:, :, 2, 0, :, :, 2, 0]\
                           +self.G4r[:, :, 1, 3, :, :, 1, 3] + self.G4r[:, :, 1, 3, :, :, 3, 1] \
                           + self.G4r[:, :, 3, 1, :, :, 1, 3] + self.G4r[:, :, 3, 1, :, :, 3, 1]) ## sum dx2y2,dz2

            G4spm1 = 0.25 * (self.G4r[:, :, 0, 2, :, :, 0, 2] + self.G4r[:, :, 0, 2, :, :, 2, 0] \
                           + self.G4r[:, :, 2, 0, :, :, 0, 2] + self.G4r[:, :, 2, 0, :, :, 2, 0]) ## only dx2y2

            G4spm2 = 0.25 *(self.G4r[:, :, 1, 3, :, :, 1, 3] + self.G4r[:, :, 1, 3, :, :, 3, 1] \
                           + self.G4r[:, :, 3, 1, :, :, 1, 3] + self.G4r[:, :, 3, 1, :, :, 3, 1]) ## only dz2

            G4d = (self.G4r[:, :, 0, 0, :, :, 0, 0] + self.G4r[:, :, 2, 2, :, :, 2, 2] \
                  + self.G4r[:, :, 1, 1, :, :, 1, 1] + self.G4r[:, :, 3, 3, :, :, 3, 3])/2. ## sum dx2y2, dz2
            G4d1 = (self.G4r[:, :, 0, 0, :, :, 0, 0] + self.G4r[:, :, 2, 2, :, :, 2, 2])/2. ## only dx2y2
            G4d2 = (self.G4r[:, :, 1, 1, :, :, 1, 1] + self.G4r[:, :, 3, 3, :, :, 3, 3])/2. ## only dz2

        Ps = sum(G4spm) / (float(self.Nc) * self.invT)
        Ps1 = sum(G4spm1) / (float(self.Nc) * self.invT)
        Ps2 = sum(G4spm2) / (float(self.Nc) * self.invT)
 
        for l1 in range(nOrb):
           for l2 in range(nOrb):
               for l3 in range(nOrb):
                   for l4 in range(nOrb):        
                       csumspmc[l1,l2,l3,l4] = sum(self.G4r[:,:,l1,l2,:,:,l3,l4])/(float(self.Nc) * self.invT)
                       print('clsuter spm', l1,l2,l3,l4, self.Tval, ' ', real(csumspmc[l1, l2, l3, l4]))

        print("G4 spm cluster Pairfield susceptibility Total: ", real(Ps))
        print("G4 spm cluster Pairfield susceptibility dx2y2: ", real(Ps1))
        print("G4 spm cluster Pairfield susceptibility dz2  : ", real(Ps2))
        '''
    ########################################################################
    ...
    def calcLatticeSCSus(self):
        ...
        csumxy = np.zeros((nOrb, nOrb, nOrb, nOrb), dtype='complex')
        ...
        csumxy1 = np.zeros((nOrb, nOrb, nOrb, nOrb), dtype='complex')
                                            ...
                                            for l6 in range(nOrb):
                                                for l7 in range(nOrb):
                                                    for l8 in range(nOrb):
                                                        ...
                                                        #                           self.chi0[iw2, iK2, l7, l8, iw2, iK2, l3, l4]
        '''                                                       
        csum = np.einsum('wkabwkef,wkefmngh,mnghmncd -> abcd', self.chi0D, self.GammaRed, self.chi0D)
      
        csumxy = np.einsum('wkabwkef,wkefmngh,mnghmncd -> abcd', self.chi0DXY, self.GammaRed, self.chi0DXY)

        csumxs = np.einsum('wkabwkef,wkefmngh,mnghmncd -> abcd',self.chi0XS, self.GammaRed, self.chi0XS)

        ccsum = np.einsum('k,wkabwkef,wkefmngh,mnghmncd,n -> abcd',gkd, self.chi0, self.GammaRed, self.chi0, gkd)

        csumspm = np.einsum('wkabwkef,wkefmngh,mnghmncd -> abcd',self.chi0, self.GammaRed, self.chi0)

        csum[:, :, :, :] /= (self.Nc * self.invT) ** 2
        csumxy[:, :, :, :] /= (self.Nc * self.invT) ** 2
        ...
        self.chi0DXY2sim = np.zeros((NwG4, Nc, nOrb, nOrb, nOrb, nOrb), dtype='complex')
        ...
        for iw in range(NwG4):
            for iK in range(Nc):
                ...
                self.chi0DXY2sim[iw, iK, :, :, :, :] = self.chi0DXY2[iw, iK, :, :, iw, iK, :, :]
            ...
            for l2 in range(nOrb):
                for l3 in range(nOrb):
                    for l4 in range(nOrb):
                        ...
                        csumxy1[l1, l2, l3, l4] = sum(self.chi0DXY2sim[:, :, l1, l2, l3, l4]) / (self.Nc * self.invT)
        ...
        if self.model == 'La3Ni2O7':
            ...
            for l1 in range(nOrb):
                for l2 in range(nOrb): 
                      Pd += csum[l1, l1, l2, l2]/4.
                      Pdxy += csumxy[l1, l1, l2, l2]
            ##for d pair-field susceptibility 
            #Pd0  = csum[0, 0, 0, 0]+csum[1, 1, 1, 1]+csum[0, 0, 1, 1]+csum[2, 2, 3, 3]+\
            #       csum[0, 0, 3, 3]+csum[1, 1, 3, 3]+csum[0, 0, 2, 2]+csum[1, 1, 2, 2]   ##sum the two orbitals
            #Pd1 = csum[0, 0, 0, 0]+csum[2, 2, 2, 2]    ## only dx2y2
            #Pd2 = csum[1, 1, 1, 1]+csum[3, 3, 3, 3]    ## only dz2

            ##for d pair-field susceptibility with the Cluster form-factors
            #Pdc0  =  ccsum[0, 0, 0, 0]+ccsum[1, 1, 1, 1]+ccsum[0, 0, 1, 1]+ccsum[2, 2, 3, 3]+\
            ...
            nOrb = self.nOrb
            Pspm = 0; Pspmx = 0; Pspmz = 0; self.csumspmK = np.zeros((2, 2), dtype='complex')
            self.csumspmKx = np.zeros((2, 2), dtype='complex')
            self.csumspmKz = np.zeros((2, 2), dtype='complex')
            for l1 in range(self.nOrb):
                for l2 in range(self.nOrb):
                    for l3 in range(self.nOrb):
                        for l4 in range(self.nOrb):
                            if self.vertex_channel in ("PARTICLE_PARTICLE_UP_DOWN"):
                                self.csumspmK[0,0] += csumspm[l1, l2, l3, l4] * Matrix[0][0][l1][l2] * Matrix[0][0][l3][l4]+\
                                                      csumspm[l1, l2, l3, l4] * Matrix[1][1][l1][l2] * Matrix[1][1][l3][l4]+\
                                                      csumspm[l1, l2, l3, l4] * Matrix[0][0][l1][l2] * Matrix[1][1][l3][l4]+\
                                                      csumspm[l1, l2, l3, l4] * Matrix[1][1][l1][l2] * Matrix[0][0][l3][l4]

                                self.csumspmK[0,1] += csumspm[l1, l2, l3, l4] * Matrix[0][0][l1][l2] * Matrix[2][2][l3][l4]+\
                                                      csumspm[l1, l2, l3, l4] * Matrix[1][1][l1][l2] * Matrix[3][3][l3][l4]+\
                                                      csumspm[l1, l2, l3, l4] * Matrix[1][1][l1][l2] * Matrix[2][2][l3][l4]+\
                                                      csumspm[l1, l2, l3, l4] * Matrix[0][0][l1][l2] * Matrix[3][3][l3][l4]

                                self.csumspmK[1,0] += csumspm[l1, l2, l3, l4] * Matrix[3][3][l1][l2] * Matrix[0][0][l3][l4]+\
                                                      csumspm[l1, l2, l3, l4] * Matrix[3][3][l1][l2] * Matrix[1][1][l3][l4]+\
                                                      csumspm[l1, l2, l3, l4] * Matrix[2][2][l1][l2] * Matrix[0][0][l3][l4]+\
                                                      csumspm[l1, l2, l3, l4] * Matrix[2][2][l1][l2] * Matrix[1][1][l3][l4]

                                self.csumspmK[1,1] += csumspm[l1, l2, l3, l4] * Matrix[2][2][l1][l2] * Matrix[2][2][l3][l4]+\
                                                      csumspm[l1, l2, l3, l4] * Matrix[3][3][l1][l2] * Matrix[3][3][l3][l4]+\
                                                      csumspm[l1, l2, l3, l4] * Matrix[2][2][l1][l2] * Matrix[3][3][l3][l4]+\
                                                      csumspm[l1, l2, l3, l4] * Matrix[3][3][l1][l2] * Matrix[2][2][l3][l4]

                                self.csumspmKx[0,0] += csumspm[l1, l2, l3, l4] * Matrix[0][0][l1][l2] * Matrix[0][0][l3][l4]
                                self.csumspmKx[0,1] += csumspm[l1, l2, l3, l4] * Matrix[0][0][l1][l2] * Matrix[2][2][l3][l4]
                                self.csumspmKx[1,0] += csumspm[l1, l2, l3, l4] * Matrix[2][2][l1][l2] * Matrix[0][0][l3][l4]
                                self.csumspmKx[1,1] += csumspm[l1, l2, l3, l4] * Matrix[2][2][l1][l2] * Matrix[2][2][l3][l4]

                                self.csumspmKz[0,0] += csumspm[l1, l2, l3, l4] * Matrix[1][1][l1][l2] * Matrix[1][1][l3][l4]
                                self.csumspmKz[0,1] += csumspm[l1, l2, l3, l4] * Matrix[1][1][l1][l2] * Matrix[3][3][l3][l4]
                                self.csumspmKz[1,0] += csumspm[l1, l2, l3, l4] * Matrix[3][3][l1][l2] * Matrix[1][1][l3][l4]
                                self.csumspmKz[1,1] += csumspm[l1, l2, l3, l4] * Matrix[3][3][l1][l2] * Matrix[3][3][l3][l4]
            for i in range(2):
                for j in range(2):
                    if self.vertex_channel in ("PARTICLE_PARTICLE_UP_DOWN"):
                        Pspm += 0.25*self.csumspmK[i,j] * cos(i*np.pi) * cos(j*np.pi)
                        Pspmx += 0.25*self.csumspmKx[i,j] * cos(i*np.pi) * cos(j*np.pi)
                        Pspmz += 0.25*self.csumspmKz[i,j] * cos(i*np.pi) * cos(j*np.pi)
            Pspm1 = Pspm - Pspmx - Pspmz
        ...
        for l1 in range(nOrb):
           for l2 in range(nOrb):
               for l3 in range(nOrb):
                   for l4 in range(nOrb):
                       print('lattice spm', l1,l2,l3,l4, self.Tval, ' ', real(csumspm[l1, l2, l3, l4]))
        '''
 
        print('Total ', 'spm-wave pair-feild susceptibility lattice:    ', self.Tval, ' ', real(Pspm))
        print('dx2y2 ', 'spm-wave pair-feild susceptibility lattice:    ', self.Tval, ' ', real(Pspmx))
        print('dz2 ', 'spm-wave pair-feild susceptibility lattice:    ', self.Tval, ' ', real(Pspmz))
        print('Else ', 'spm-wave pair-feild susceptibility lattice:    ', self.Tval, ' ', real(Pspm1))
        
        print('Total', 'd-wave pair-feild susceptibility : ', self.Tval, ' ', real(Pd))
        print('Total', 'dxy-wave pair-feild susceptibility : ', self.Tval, ' ', real(Pdxy))
        #print('Total', 'd-wave pair-feild susceptibility : ', self.Tval, ' ', real(Pd0))
        #print('Total', 'd-wave pair-feild susceptibility with cluster form-factor: ', self.Tval, ' ', real(Pdc0))
...
###################################################################### writting functions
    ...
    def write_data_7cols(self, fname, xs, ys, zs, ws, fs, gs, hs):
        f = open(fname,'a',1)
        for i in range(len(xs)):
            f.write('{:.6e}\t{:.6e}\t{:.6e}\t{:.6e}\t{:.6e}\t{:.6e}\t{:.6e}\n'.format(float(xs),float(ys),float(zs),float(ws.real),float(fs.real),float(gs.real),float(hs.real)))
                           
###################################################################################
Ts = [1, 0.75, 0.5, 0.44, 0.4, 0.34, 0.3, 0.25, 0.24, 0.225, 0.2, 0.175, 0.17, 0.15, 0.125, 0.1, 0.09, 0.08, 0.07, 0.06, 0.05, 0.04, 0.035, 0.03, 0.025,0.02,0.01]
#Ts = [0.05,0.04,0.03,0.02,0.01]#, 0.02]
...
for T_ind, T in enumerate(Ts):
    ...
    for v in Nv:
        for q in qs:
            ...
            #file_tp = './T='+str(Ts[T_ind])+'/dca_tp_mag_q00'+'.hdf5'
